(function(){
  ...
  function hideOld(){['#nsThemeToggle','#nsMusicToggle','.helpfloat','.float','.back'].forEach(function(sel){document.querySelectorAll(sel).forEach(function(el){el.style.display='none'});});}
  ...
  function make(){
    ...
    var panel=document.createElement('div');panel.id='nsControlPanel';panel.style.cssText='display:none;position:fixed;right:14px;bottom:calc(var(--ns-float-bottom,96px) + 54px);z-index:12000;width:min(260px,calc(100vw - 28px));padding:12px;border-radius:22px;background:rgba(255,250,242,.96);color:#24170f;box-shadow:0 22px 70px rgba(0,0,0,.22);border:1px solid rgba(217,164,65,.25)';
    function render(){
      panel.innerHTML='<b style="display:block;margin:0 0 8px">Panel Kontrol</b>'+'<button id="nsPanelTheme" style="width:100%;margin:5px 0;padding:12px;border-radius:14px;border:0;font-weight:900">Tema: '+labelTheme()+'</button>'+'<button id="nsPanelMusic" style="width:100%;margin:5px 0;padding:12px;border-radius:14px;border:0;font-weight:900">'+musicLabel()+'</button>'+'<a style="display:block;text-align:center;text-decoration:none;margin:5px 0;padding:12px;border-radius:14px;font-weight:900;background:#9b2f20;color:white" href="'+linkFor('.helpfloat,.float','safety/index.html?v=18')+'">Bantuan</a>'+'<a style="display:block;text-align:center;text-decoration:none;margin:5px 0;padding:12px;border-radius:14px;font-weight:900;background:rgba(217,164,65,.35);color:#24170f" href="'+linkFor('.back','gerbang-10.html?v=18')+'">Gerbang</a>';
      ...
      if(document.documentElement.getAttribute('data-theme')==='dark'){panel.style.background='rgba(43,31,25,.96)';panel.style.color='#f6ead7'}else{panel.style.background='rgba(255,250,242,.96)';panel.style.color='#24170f'}
    }
    btn.onclick=function(){hideOld();render();panel.style.display=panel.style.display==='none'?'block':'none'};
    document.body.appendChild(panel);document.body.appendChild(btn);setInterval(hideOld,1200);
  }
