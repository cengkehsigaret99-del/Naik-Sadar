  ...
  function inject(){
    ...
    if(!doc.getElementById('rsFrameNavStyle')){
      ...
      style.textContent = `
        ...
        .ns-cultural-shell{padding-bottom:120px!important}
        .rs-frame-nav{position:fixed;left:50%;bottom:calc(10px + env(safe-area-inset-bottom,0px));transform:translateX(-50%);z-index:9998;width:min(620px,calc(100% - 16px));display:grid;grid-template-columns:repeat(7,1fr);gap:4px;padding:7px;border-radius:24px;background:rgba(255,248,234,.92);border:1px solid rgba(75,46,31,.16);box-shadow:0 18px 44px rgba(75,46,31,.18);backdrop-filter:blur(10px)}
        .rs-frame-nav a{min-height:45px;display:flex;align-items:center;justify-content:center;text-align:center;border-radius:16px;text-decoration:none;color:#6f5b48;font-size:10.5px;font-weight:950;letter-spacing:.01em;line-height:1.05}
        ...
        html[data-theme="dark"] .ns-gapura-hero,html[data-theme="dark"] .ns-gunungan-panel,html[data-theme="dark"] .ns-lontar-card,html[data-theme="dark"] .ns-sajadah-card,html[data-theme="dark"] .ns-ritual-step,html[data-theme="dark"] .ns-batin-note,html[data-theme="dark"] .mirror-card,html[data-theme="dark"] .napas-panel,html[data-theme="dark"] .rapi-card,html[data-theme="dark"] .release-card,html[data-theme="dark"] .check-card{background:rgba(36,25,18,.90)!important;border-color:rgba(244,216,138,.18)!important;box-shadow:0 18px 48px rgba(0,0,0,.32)!important;color:#fff8ea!important}
        ...
        @media(max-width:430px){.rs-frame-nav a{font-size:9.5px;min-height:42px}.rs-frame-nav{gap:3px;padding:6px;border-radius:22px}}
        @media(max-width:340px){.rs-frame-nav{grid-template-columns:repeat(4,1fr)}body{padding-bottom:150px!important}.ns-cultural-shell{padding-bottom:168px!important}}
