  ...
  function inject(){
    ...
    if(!doc.getElementById('rsFrameNavStyle')){
      ...
      style.textContent = `
        ...
        .ns-cultural-shell{padding-bottom:140px!important}
        .rs-frame-nav{position:fixed;left:50%;bottom:calc(10px + env(safe-area-inset-bottom,0px));transform:translateX(-50%);z-index:9998;width:min(700px,calc(100% - 14px));display:grid;grid-template-columns:repeat(9,1fr);gap:3px;padding:7px;border-radius:24px;background:rgba(255,248,234,.92);border:1px solid rgba(75,46,31,.16);box-shadow:0 18px 44px rgba(75,46,31,.18);backdrop-filter:blur(10px)}
        .rs-frame-nav a{min-height:44px;display:flex;align-items:center;justify-content:center;text-align:center;border-radius:16px;text-decoration:none;color:#6f5b48;font-size:9.5px;font-weight:950;letter-spacing:.01em;line-height:1.05}
        ...
        html[data-theme="dark"] .rs-card-link,html[data-theme="dark"] .lontar-link,html[data-theme="dark"] .laku-card,html[data-theme="dark"] .release-link,html[data-theme="dark"] .check-row,html[data-theme="dark"] .journal,html[data-theme="dark"] .ritual-step,html[data-theme="dark"] .check,html[data-theme="dark"] .time-row,html[data-theme="dark"] .voice-item,html[data-theme="dark"] .item,html[data-theme="dark"] .empty{background:rgba(255,248,234,.07)!important;border-color:rgba(244,216,138,.16)!important;color:#fff8ea!important}
        ...
        @media(max-width:430px){.rs-frame-nav a{font-size:8.8px;min-height:40px}.rs-frame-nav{gap:3px;padding:6px;border-radius:22px}}
        @media(max-width:380px){.rs-frame-nav{grid-template-columns:repeat(5,1fr)}body{padding-bottom:170px!important}.ns-cultural-shell{padding-bottom:190px!important}}
